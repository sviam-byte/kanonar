...
/**
 * Resolve the emotion curve preset for a specific emotion key, falling back to global defaults.
 */
function pickPreset(world: any, selfId: string, key: string): CurvePreset {
  ...
  const p = typeof v === 'string' && ok.has(v) ? v : typeof g === 'string' && ok.has(g) ? g : 'smoothstep';
...
export function deriveAppraisalAndEmotionAtomsV2(
  ...
  const emoAtoms: ContextAtom[] = [
    ...
    mk(`emo:relief:${selfId}`, relief, usedEmo, { aThreat, aControl, raw: rawRelief, preset: reliefPreset, relief }, 'emotion'),
    mk(
      `emo:resolve:${selfId}`,
      resolve,
      usedEmo,
    ...
    mk(`emo:care:${selfId}`, care, usedEmo, { aAttach, aThreat, raw: rawCare, preset: carePreset, care }, 'emotion'),
