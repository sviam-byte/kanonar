...
function coerceProvenanceFromAtom(atom: any, stageId: string): Provenance[] | undefined {
  ...
  const used = arr<string>(tr.usedAtomIds).filter((x) => typeof x === 'string' && x.length > 0);
  const parts = tr.parts;
  const notes = arr<string>(tr.notes).filter((x) => typeof x === 'string' && x.length > 0);
...
function buildArtifactsFromFrame(frame: GoalLabStageFrame, run: GoalLabPipelineV1): ArtifactRef[] {
  ...
  const atoms = arr<any>(frame.atoms).map((a: Record<string, unknown>) => ({
    ...a,
  ...
  const v1: Record<string, unknown> = frame.artifacts && typeof frame.artifacts === 'object' ? frame.artifacts as Record<string, unknown> : {};
  ...
  if (stageId === 'S0') {
    ...
    const pickByPrefix = (allow: Set<string>) =>
      atoms.filter((a: Record<string, unknown>) => {
    ...
    const obsSnap = v1.observationSnapshot as Record<string, unknown> | undefined;
    ...
    if (v1.obsAtomsCount != null || v1.provenanceSize != null || v1?.observationSnapshot != null) {
      push('observation', 'observation_summary', 'Observation summary', {
        obsAtomsCount: v1.obsAtomsCount ?? null,
        provenanceSize: v1.provenanceSize ?? null,
        observationSnapshot: v1.observationSnapshot ?? null,
      });
      if (v1?.observationSnapshot != null) {
        push('observation', 'observation_snapshot', 'ObservationSnapshot (S0)', v1.observationSnapshot);
      }
    }

    if (v1?.beliefUpdateSnapshot != null) {
      push('belief', 'belief_update_snapshot', 'BeliefUpdateSnapshot (lite, S0)', v1.beliefUpdateSnapshot);
    }
  }

  // S1: quarks are compact observation features.
  if (stageId === 'S1' && v1.quarks != null) {
    push('observation', 'quarks', 'Quarks', v1.quarks);
  }

  // S2-S4: context/lens/emotions shape belief state.
  if (stageId === 'S2') {
    push('belief', 'context_axes', 'Context axes / enrichers', {
      socialProximity: v1.socialProximity ?? null,
      hazardGeometry: v1.hazardGeometry ?? null,
      ctxAxisCount: v1.ctxAxisCount ?? null,
      overriddenIds: v1.overriddenIds ?? [],
      moduleAdds: v1.moduleAdds ?? null,
    });
  }
  if (stageId === 'S3') {
  ...
  if (stageId === 'S4') {
    push('belief', 'appraisal_emotions', 'Appraisal → Emotions', {
      appCount: v1.appCount ?? null,
      emoCount: v1.emoCount ?? null,
      dyEmoCount: v1.dyEmoCount ?? null,
      overriddenIds: v1.overriddenIds ?? [],
    });
  }
  ...
  if (stageId === 'S5') {
    push('tom', 'tom_summary', 'ToM summary', {
      relPriorsCount: v1.relPriorsCount ?? null,
      nonContextDyadCount: v1.nonContextDyadCount ?? null,
      beliefBiasCount: v1.beliefBiasCount ?? null,
      policyCount: v1.policyCount ?? null,
      overriddenIds: v1.overriddenIds ?? [],
    });
  }
  ...
  if (stageId === 'S6') {
    push('belief', 'contextmind', 'ContextMind / Drivers', {
      contextMind: v1.contextMind ?? null,
      drvCount: v1.drvCount ?? null,
      overriddenIds: v1.overriddenIds ?? [],
    });
  }
  ...
  if (stageId === 'S7') {
    const gls = v1.goalLayerSnapshot ?? null;
    if (gls) {
    ...
    push('goals', 'goals', 'Goals (ecology + planning)', {
      goalAtomsCount: v1.goalAtomsCount ?? null,
      goalDebug: v1.goalDebug ?? null,
      goalLayerSnapshot: v1.goalLayerSnapshot ?? null,
      planGoalAtomsCount: v1.planGoalAtomsCount ?? null,
      goalActionLinksCount: v1.goalActionLinksCount ?? null,
      utilAtomsCount: v1.utilAtomsCount ?? null,
      topPlanGoals: v1.topPlanGoals ?? [],
      overriddenIds: v1.overriddenIds ?? [],
    });
  }
  ...
  if (stageId === 'S8') {
    if (v1.error) {
      push('decision', 'decision_error', 'Decision error', v1.error);
    }
    if (v1.modesSnapshot) {
      push('modes', 'modes', 'ModesSnapshot', v1.modesSnapshot);
    }
    if (v1.stabilizersSnapshot) {
      push('stabilizers', 'stabilizers', 'StabilizersSnapshot', v1.stabilizersSnapshot);
    }
    if (v1.decisionSnapshot != null) {
      push('decision', 'decision_snapshot', 'DecisionSnapshot (breakdown)', v1.decisionSnapshot);
    }
    push('decision', 'decision', 'Decision (ranked/best/intent)', {
      accessDecisions: v1.accessDecisions ?? [],
      ranked: v1.ranked ?? [],
      best: v1.best ?? null,
      decisionSnapshot: v1.decisionSnapshot ?? null,
      intentPreview: v1.intentPreview ?? null,
      overriddenIds: v1.overriddenIds ?? [],
      priorsAtomIds: v1.priorsAtomIds ?? [],
      decisionAtomIds: v1.decisionAtomIds ?? [],
    });
  }
  ...
  const stages: PipelineStage[] = arr<GoalLabStageFrame>(p.stages).map((s) => {
    const stats =
      s.stats && typeof s.stats === 'object'
        ? {
            atomCount: Number(s.stats?.atomCount ?? 0),
            addedCount: Number(s.stats?.addedCount ?? 0),
            missingCodeCount: Number(s.stats?.missingCodeCount ?? 0),
            missingTraceDerivedCount: Number(s.stats?.missingTraceDerivedCount ?? 0),
          }
        : undefined;
