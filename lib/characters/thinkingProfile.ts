...
function avg(...xs: number[]) {
  ...
  let sum = 0;
  for (const x of xs) sum += Number.isFinite(x) ? x : 0;
  return sum / xs.length;
}

function V(c: CharacterEntityLike, key: string, fallback = 0.5) {
  // Character vectors live in vector_base (project convention).
...
function S(c: CharacterEntityLike, key: string, fallback = 0.5) {
  return clamp01(num(c.skills?.[key], fallback));
}

/**
 * Deterministic “character sheet” metric.
 * Pure function of the character entity (no simulation tick inputs).
 */
export function deriveThinkingAndActivityFromCharacter(c: CharacterEntityLike): {
  thinking: ThinkingProfile;
  activityCaps: ActivityCaps;
} {
  // --- Sources (prefer explicit latents/skills; fallback to vector_base) ---
  const formal = avg(L(c, 'FORMAL', 0.5), V(c, 'E_Model_calibration', 0.5), V(c, 'A_Legitimacy_Procedure', 0.5));
  const action = avg(L(c, 'ACTION', 0.5), V(c, 'E_Skill_ops_fieldcraft', 0.5), V(c, 'D_fine_motor', 0.5));
  ...
  // --- Activity capabilities (static; derived from thinking + traits) ---
  const activityCaps: ActivityCaps = {
    operations: clamp01(avg(action, V(c, 'B_cooldown_discipline', 0.5))),
    ...
    sensorimotor: clamp01(avg(thinking.representation.enactive, action)),
    instrumental: clamp01(avg(thinking.representation.verbal, thinking.representation.formal, verbal)),
    communicative: clamp01(avg(tom, S(c, 'diplomacy', 0.5), V(c, 'E_Skill_diplomacy_negotiation', 0.5))),
