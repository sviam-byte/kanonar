...
export function getDyadTrust(world: SimWorld, fromId: string, toId: string): number {
  // Try known fact keys in decreasing priority.
  ...
  // Fallback: if you have a relations matrix-like object.
...
export function decideAcceptance(
  ...
  const compat = (vL && vS) ? (0.5 + 0.5 * cosine(vL, vS)) : 0.6; // map [-1..1] -> [0..1] with fallback
  ...
  const p = sigmoid(3.2 * (x - 0.5)); // sharper around 0.5
