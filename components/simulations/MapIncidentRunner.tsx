...
export const MapIncidentRunner: React.FC<MapIncidentRunnerProps> = ({ sim }) => {
    ...
    const [grid, setGrid] = useState<number[][]>(() => createInitialGrid(w, h));

    const animationFrameRef = useRef<number>();
    const lastUpdateTimeRef = useRef<number>(0);
    const simulationSpeed = 250; // ms per day
    ...
    const advanceDay = useCallback(() => {
        setDay(prevDay => {
            ...
            setGrid(currentGrid => runSimulationStep(currentGrid, payload, nextDay));
            return nextDay;
        });
    }, [payload]);
