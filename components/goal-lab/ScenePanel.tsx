...
export const ScenePanel: React.FC<Props> = ({ control, presets, onChange }) => {
  const withIds = presets.map(p => ({ ...p, id: p.id ?? p.presetId }));
  const preset = withIds.find(p => p.id === control.presetId) || withIds[0];
      ...
      <div className="p-3 border-b border-canon-border bg-canon-bg-light/30">
        ...
        <div className="mt-3 flex gap-2">
          <div className="flex-1">
              ...
              >
                {withIds.map(p => <option key={p.id} value={p.id}>{p.title}</option>)}
              </select>
          </div>
