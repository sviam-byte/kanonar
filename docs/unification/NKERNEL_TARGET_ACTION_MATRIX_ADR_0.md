...
3. **NKERNEL-TARGET-MATRIX-DECISION-0** — ✅ IMPLEMENTED 2026-07-22:
   per-target projection fan-out, independent S8/RNG frames, cell traces и
   directed reference lane.
4. **NKERNEL-TARGET-MATRIX-SESSION-0** — ✅ IMPLEMENTED 2026-07-22: additive
   N≥2 live loop, persistent per-cell RNG, directed replay/history и N=2
   reduction; старый session wrapper не изменён.
5. **NKERNEL-TARGET-MATRIX-UI-0** — ✅ IMPLEMENTED 2026-07-22:
   explicit-opt-in Dyad/Triad visualization, per-round action matrices,
   final directed relations, per-cell traces и отдельная versioned export
   schema; UI не является source of truth.

`runConflictNJointDecisionV1` и `runConflictNLabSessionV1` сохраняют свои
dyad-only typed errors при `N > 2`; новый matrix runtime имеет отдельные
versioned entrypoints и не меняет их публичную семантику.
...
- Matrix session принимает только canonical `trust_exchange`, `N >= 2` и
  round budgets `1..30`; replay использует `scheduledStates`, а фактическое
  расписание сохраняется в `pressureHistory`.
- N=2 сохраняет прежние seed/channel IDs; N>2 использует collision-free
  target-qualified IDs и отдельный mutable RNG object для каждой directed cell.
- Каждый раунд содержит полные matrix/history и `N * (N - 1)` decision traces.
