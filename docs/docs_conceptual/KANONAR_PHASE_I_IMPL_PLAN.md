...
| `legacy` | все механизмы профиля OFF | явная контрольная ветка UI |
| `phase1` | communication threat, object context, location props, threat memory, prior influence, PAM v2, S5 OpponentBelief dual-emit, active-goal/domain energy union | обычный пользовательский запуск с наблюдаемыми механизмами |
