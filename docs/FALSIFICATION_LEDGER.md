...
| MVP0-CORE | The living cycle exists (A1+A2 min-PASS): rollout of 2 live agents × 20 ticks with a world that changes; every action explainable and counterfactable | CRAFT/impl | **RAN 2026-07-07 (freeze 54c4c74): PASS; golden provenance corrected in freeze 626c4d3** | `runMvpRollout(seed 7, 20 ticks)`: golden hash `124e3434…e5a7f89c` reproduces byte-identically (A1); 40/40 rows with non-empty menu AND non-empty usedAtomIds (26–44 atoms per choice, A2); agents interact (help/encourage/negotiate, natural speech:v1 at t4); `runTwins` names first divergence tick + the diverged atoms (injectSpeechAtom: diverges t0, twin-side atoms include `event:threaten:evt:speech:inject:…`). Audit correction: clean checkouts of freeze `54c4c74` and later commits reproduce `124e3434…`; ran commit `3dd9cf3` had accidentally pinned unsupported `73eaf2ce…` | pin breaks loudly on any behavioral engine change; the corrected hash is verified by `tests/simkit/mvp0_golden.test.ts` + `mvp0_twins.test.ts` |
