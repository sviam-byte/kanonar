...
| MVP0-CORE | The living cycle exists (A1+A2 min-PASS): rollout of 2 live agents × 20 ticks with a world that changes; every action explainable and counterfactable | CRAFT/impl | **RAN 2026-07-07 (freeze 54c4c74): PASS** | `runMvpRollout(seed 7, 20 ticks)`: golden hash `73eaf2ce…2acb0fdd` reproduced byte-identically (A1); 40/40 rows with non-empty menu AND non-empty usedAtomIds (26–44 atoms per choice, A2); agents interact (help/encourage/negotiate, natural speech:v1 at t4); `runTwins` names first divergence tick + the diverged atoms (injectSpeechAtom: diverges t0, twin-side atoms include `event:threaten:evt:speech:inject:…`). Assembly repairs (in the freeze): agentAtoms had NO S0 consumer; extractAgentTrace read `why` through a nonexistent wrapper (usedAtomIds silently empty); colon-bearing agent ids corrupt possibility-id parsing | pin breaks loudly on any behavioral engine change (that is its job) · `tests/simkit/mvp0_golden.test.ts` + `mvp0_twins.test.ts` |
