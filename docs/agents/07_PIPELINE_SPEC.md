# 7) Pipeline Spec — стадии S0…S8 и контракты namespaces
...
## 7.11. S8 — Outcome / World update

**Назначение:** применить выбранное действие и обновить `world/obs` на следующий тик.

Контракт:

- любые derived-изменения должны либо стать новым `world:*`/`obs:*`, либо быть сброшены (иначе копится мусор и ломается объяснимость).
