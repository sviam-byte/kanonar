# 7) Pipeline Spec — историческая схема `runGoalLabPipelineV1` (S0…S8)

> Статус: историческая подробная глава. Текущая реализация имеет стадии
> `S0...S9`, включая S4 Emotions и S9 Predict tick. Канонический контракт —
> `docs/PIPELINE.md`; при расхождении он и runtime имеют приоритет.
