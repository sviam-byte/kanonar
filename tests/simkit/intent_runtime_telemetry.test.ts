...
describe('intent runtime telemetry facts', () => {
  it('writes telemetry when stale intent is aborted during continue', () => {
    ...
    const evt = (out.world.facts as any)['sim:intent:last:A'];
    expect(evt?.event).toBe('stale_abort');
    expect(evt?.intentId).toBe('intent:A:old');
    expect(evt?.stageKind).toBe('approach');
  });
});
